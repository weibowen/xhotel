 ...
 */
Ext
		.define('MyApp.ux.TimePickerField',
				{
					...
					initComponent : function() {
						...
						// TODO 使用timeformat 判断是否创建秒输入框, maybe second field is
						// not always need.
					...
					onRender : function() {
						...
						// spinnerWrap = Ext.get(Ext.DomQuery.selectNode('div',
						// this.el.dom)); // 4.0.2
						spinnerWrapDom = Ext.dom.Query.select('td', this
								.getEl().dom)[1]; // 4.0 ->4.1 div->td
					...
					setValue : function(value) {
						this.value = Ext.isDate(value) ? Ext.Date.format(value,
								'H:i:s') : value;
						if (!this.rendered) {
							return;
						}
